...
// A GarbageCollector garbage collects watches for a single managed resource
...
// types that are no longer composed by any managed resource (XR).
func (gc *GarbageCollector) GarbageCollectWatches(ctx context.Context, interval time.Duration) {
	...
	for {
		select {
		case <-ctx.Done():
			gc.log.Debug("Stopping managed controller watch garbage collector", "error", ctx.Err())
			return
		case <-t.C:
			if err := gc.GarbageCollectWatchesNow(ctx); err != nil {
				gc.log.Info("Cannot garbage collect managed controller watches", "error", err)
			}
		}
	}
}
...
// longer composed by any managed resource (XR). It's safe to call from
// multiple goroutines.
func (gc *GarbageCollector) GarbageCollectWatchesNow(ctx context.Context) error {
	...
	return errors.Wrap(err, "cannot stop watches for composed resource types that are no longer referenced by any managed resource")
}
