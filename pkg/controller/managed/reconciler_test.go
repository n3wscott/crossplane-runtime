...
func TestReconcile(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"CompositeResourceNotFound": {
			reason: "We should not return an error if the managed resource was not found.",
			args: args{
				c: &test.MockClient{
		...
		"GetCompositeResourceError": {
			reason: "We should return error encountered while getting the managed resource.",
			args: args{
				c: &test.MockClient{
		...
		"ConfigureCompositeError": {
			reason: "We should return any error encountered while configuring the managed resource.",
			args: args{
				c: &test.MockClient{
					MockGet: test.NewMockGetFn(nil),
					MockStatusUpdate: WantComposite(t, NewComposite(func(cr resource.Composite) {
		...
		"ReconciliationResumes": {
			reason: `If a managed resource has the pause annotation with some value other than "true" and the Synced=False/ReconcilePaused status condition, reconciliation should resume with requeuing.`,
			args: args{
				c: &test.MockClient{
					MockGet: WithComposite(t, NewComposite(func(cr resource.Composite) {
		...
		"ReconciliationResumesAfterAnnotationRemoval": {
			reason: `If a managed resource has the pause annotation removed and the Synced=False/ReconcilePaused status condition, reconciliation should resume with requeuing.`,
			args: args{
				c: &test.MockClient{
					MockGet: WithComposite(t, NewComposite(func(cr resource.Composite) {
						// no annotation atm
		...
		"CustomEventsAndConditions": {
			reason: "We should emit custom events and set custom conditions that were returned by the composer on both the managed resource and the claim.",
			args: args{
				c: &test.MockClient{
					...
					WithRecorder(newTestRecorder(
						...
						eventArgs{
							...
							Event: event.Event{
								...
								// The managed should have the "Pipeline step" prefix.
		...
		"CustomEventsAndConditionFatal": {
			reason: "In the case of a fatal result from the composer, we should set all custom conditions that were seen. If any custom conditions were not seen, they should be marked as Unknown. The error message should be emitted as an event to the managed but not the claim.",
			args: args{
				c: &test.MockClient{
					MockGet: test.NewMockGetFn(nil, func(obj client.Object) error {
						if xr, ok := obj.(*composite.Unstructured); ok {
					...
					WithRecorder(newTestRecorder(
						...
						eventArgs{
							...
							Event: event.Event{
								...
								// The managed should not have the prefix as it had an empty
								// detail.
		...
		"CustomEventsFailToGetClaim": {
			reason: "We should emit custom events that were returned by the composer. If we cannot get the claim, we should just emit events for the managed and continue as normal.",
			args: args{
				c: &test.MockClient{
					MockGet: test.NewMockGetFn(nil, func(obj client.Object) error {
						if xr, ok := obj.(*composite.Unstructured); ok {
